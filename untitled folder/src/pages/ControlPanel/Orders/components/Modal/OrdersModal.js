...
export default function OrdersModal(props) {
    ...
    return (
        <div>
            ...
            >
                <Box sx={style}>
                    <div className={styles.header}><h3>سفارش</h3><span>icon</span></div>
                    <div className={styles.infoBox}>
                        <div className={styles.minInfoBox}>  <div className={styles.txtData}><span>نام مشتري : </span><span>{data.name}</span></div>
                    ...
                    <div style={{display: "flex",justifyContent:"center",marginTop:"1rem"}}>{data.OrderStatus=="Waiting"?<Button variant="contained" color="success" align="center">تحويل شد</Button>:<><span>زمان تحويل : </span><span>{moment(data.DeliveryTime).locale('fa').format('YYYY/MM/DD')}</span></> }</div>
